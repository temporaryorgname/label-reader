...
transform = torchvision.transforms.Compose([
    ...
    data.coco.ScaleToFit(224),
    #data.coco.RandomCrop(224,224),
    data.coco.CentreCrop(224,224),
...
try:
    for samples in dataloader:
        ...
        while True:
            ...
            #lambda_noobj = obj.sum()/noobj.sum()
            #lambda_coord = 1
            #centre_loss = lambda_coord*(obj*(centre-centre_hat)**2).sum()
            #size_loss = lambda_coord*(obj*(size-centre_hat)**2).sum()
            #c_loss = (obj*(c-c_hat)**2 + lambda_noobj*noobj*(c-c_hat)**2).sum()
            #loss = centre_loss + size_loss + c_loss
            ...
            # Total loss
            epsilon = 1e-10
            loss = ((1-iou)*(-obj*torch.log(c_hat+epsilon))-lambda_noobj*noobj*torch.log(1-c_hat+epsilon)).sum()
            # Backprop
            print(loss.item(), c.sum().item(), c_hat.sum().item())
            loss.backward()
            optimizer.step()
